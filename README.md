# Go API client for flexpriceclient
...
```go
import flexpriceclient "github.com/flexprice/go-sdk/flexpriceclient"
```
...
For using other server than the one defined on index 0 set context value `flexpriceclient.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), flexpriceclient.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `flexpriceclient.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), flexpriceclient.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `flexpriceclient.ContextOperationServerIndices` and `flexpriceclient.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), flexpriceclient.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), flexpriceclient.ContextOperationServerVariables, map[string]map[string]string{
