...
// HandlePaddleWebhook - Handle Paddle webhook events
...
func (s *Webhooks) HandlePaddleWebhook(ctx context.Context, tenantID string, environmentID string, paddleSignature string, opts ...dtos.Option) (*dtos.HandlePaddleWebhookResponse, error) {
	request := dtos.HandlePaddleWebhookRequest{
		TenantID:        tenantID,
		EnvironmentID:   environmentID,
		PaddleSignature: paddleSignature,
	}
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/webhooks/paddle/{tenant_id}/{environment_id}", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	res := &dtos.HandlePaddleWebhookResponse{
		HTTPMeta: types.HTTPMetadata{
			Request:  req,
			Response: httpRes,
		},
