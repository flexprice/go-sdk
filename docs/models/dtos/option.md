...
```go
flexprice.WithServerURL("https://api.example.com")
```
...
```go
flexprice.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
flexprice.WithTimeout(30 * time.Second)
```
