...
type TasksAPITasksIdDownloadGetRequest struct {
	ctx context.Context
	ApiService *TasksAPIService
	id string
}

func (r TasksAPITasksIdDownloadGetRequest) Execute() (map[string]string, *http.Response, error) {
	return r.ApiService.TasksIdDownloadGetExecute(r)
}

/*
TasksIdDownloadGet Download task export file

Generate a presigned URL for downloading an exported file (supports both Flexprice-managed and customer-owned S3)

 @param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
 @param id Task ID
 @return TasksAPITasksIdDownloadGetRequest
*/
func (a *TasksAPIService) TasksIdDownloadGet(ctx context.Context, id string) TasksAPITasksIdDownloadGetRequest {
	return TasksAPITasksIdDownloadGetRequest{
		ApiService: a,
		ctx: ctx,
		id: id,
...
func (a *TasksAPIService) TasksIdDownloadGetExecute(r TasksAPITasksIdDownloadGetRequest) (map[string]string, *http.Response, error) {
	var (
		localVarHTTPMethod   = http.MethodGet
		localVarPostBody     interface{}
		formFiles            []formFile
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "TasksAPIService.TasksIdDownloadGet")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/tasks/{id}/download"
